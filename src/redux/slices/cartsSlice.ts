...
export const updateCart = createAsyncThunk(
  ...
  async (
    ...
    try {
      const response = await axiosInstance.put(`web/cart/update/${id}`, data);
...
export const fetchOrderDetails = createAsyncThunk(
  ...
  async ({ orderId }: { orderId: any }, thunkAPI) => {
    try {
      const res = await axiosInstance.post(`web/orders/order-details`, {
        orderId,
      });
      return res.data;
    } catch (err: any) {
...
export const removeProducts = createAsyncThunk(
  "account/updatecustomer",
  async ({ product_ids }: { product_ids: number[] }, { rejectWithValue }) => {
    try {
