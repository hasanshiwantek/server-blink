...
const CheckoutMultipleOrderSummary: React.FC<OrderSummaryProps> = ({
  ...
  const { destinations, isMultiAddress } = useAppSelector(
  ...
  const buildMultiAddressItems = () => {
    ...
    cart?.forEach((cartItem) => {
      ...
      const totalAllocated = Object.values(allocatedPerDest).reduce(
        (s, c) => s + c,
        0,
      );
      ...
      <div className="bg-white border-[1px] border-[#8b8b8b] rounded-sm shadow-sm py-6 h-fit sticky top-9">
            ...
            {visibleItems.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="text-sm font-semibold text-[#545454] shrink-0 roboto-font">
                    ${lineTotal.toFixed(2)}
                  </div>
                </div>
              );
        ...
        {!isOrderInfo && (
          <div className="mb-6 border-b-[1px] px-6 border-[#8b8b8b] py-4">
            <button
              type="button"
              className="text-[13px] text-[#D42020]"
        ...
        <div className="space-y-3  pt-4 px-6">
          ...
          {appliedCoupon && discountAmount > 0 && (
            <div className="mt-2 overflow-hidden transition-all duration-300 ease-in-out">
              <div
                className="flex text-[13px] justify-between items-center text-[#545454] cursor-pointer select-none transition-colors duration-200 hover:text-[#2b2b2b]"
                onClick={() => setDiscountOpen((prev) => !prev)}
              >
                <span className="flex items-center gap-1 font-medium">
                  Discounts
                  <svg
                    className={`w-4 h-4 transition-transform duration-300 ${discountOpen ? "rotate-180" : "rotate-0"}`}
                    fill="none"
                    stroke="currentColor"
                    strokeWidth={2}
                    viewBox="0 0 24 24"
              ...
              <div
                className={`py-4 grid transition-all duration-300 ease-in-out ${
                  discountOpen
              ...
              >
                <div className="overflow-hidden">
                  <div className="flex justify-between text-gray-600 text-[13px]">
                    <span>
                      ${Number(discountAmount).toFixed(2)} off (
