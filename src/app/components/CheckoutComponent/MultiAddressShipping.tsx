            ...
            <DialogContent className="sm:max-w-[560px] max-h-[90vh] overflow-y-auto bg-[#eeeeee]">
                ...
                <div className="space-y-4 pt-2">
                    ...
                    <div>
                        ...
                        <Input value={value.company} onChange={(e) => onChange({ ...value, company: e.target.value })} className="h-[42px] bg-white" />
                    </div>
                    <div>
                        ...
                        <Input value={value.phone} onChange={(e) => onChange({ ...value, phone: e.target.value })} className="h-[42px] bg-white" />
                    </div>
                    <div>
                        <label className="text-sm text-gray-700 mb-1 block">Address Line 1</label>
                        <Input value={value.address1} onChange={(e) => onChange({ ...value, address1: e.target.value })} className="h-[42px] bg-white" />
                    </div>
                    <div>
                        ...
                        <Input value={value.address2} onChange={(e) => onChange({ ...value, address2: e.target.value })} className="h-[42px] bg-white" />
                    </div>
                    <div>
                        <label className="text-sm text-gray-700 mb-1 block">City</label>
                        {cityList.length > 0 ? (
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="text-sm text-gray-700 mb-1 flex justify-between"><span>State/Province</span><span className="text-gray-400">(Optional)</span></label>
                            {stateList.length > 0 ? (
                                <Select value={value.state} onValueChange={(v) => onChange({ ...value, state: v, city: "", zip: "" })}>
                                    <SelectTrigger className="h-[42px] bg-white"><SelectValue placeholder="Select a state" /></SelectTrigger>
                                    <SelectContent>{stateList.map((s) => <SelectItem key={s.code} value={s.code}>{s.name}</SelectItem>)}</SelectContent>
...
// ─── Main Component ───────────────────────────────────────────────────────────
const MultiAddressShipping: React.FC<MultiAddressShippingProps> = ({
    cart, shippingRates, onComplete, onSingleAddress, onContinue,
}) => {
    ...
    return (
        <div className="space-y-4">
            ...
            {unallocatedCount > 0 && (<button type="button" onClick={handleAddDestination} className="bg-[var(--primary-color)] text-white text-sm font-medium px-4 py-2 uppercase hover:opacity-90">
                ADD NEW DESTINATION
            </button>)
            }
            <div className="flex flex-col">
            ...
            <button type="button"
                onClick={() => { if (canContinue) { dispatch(setCompletedDestinations(destinations)); onComplete(destinations); onContinue(); } }}
                disabled={!canContinue}
